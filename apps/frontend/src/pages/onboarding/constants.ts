...
export const ageOptions = Array.from({ length: 100 - 18 + 1 }, (_, i) => (i + 18).toString()).map((age) => ({ label: age, value: age }));

export const daysOptions = Array.from({ length: 7 - 1 + 1 }, (_, i) => (i + 1).toString()).map((day) => ({ label: day, value: day }));

export const heightOptions = Array.from({ length: 220 - 100 + 1 }, (_, i) => (i + 100).toString()).map((height) => ({ label: height, value: height }));

export const weightOptions = Array.from({ length: 200 - 40 + 1 }, (_, i) => (i + 40).toString()).map((weight) => ({ label: weight, value: weight }));
